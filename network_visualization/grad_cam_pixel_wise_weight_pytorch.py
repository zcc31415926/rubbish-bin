# gradient class activation mapping with pixel-wise weight
...
def draw_cam(img_path, model, cam_path, cammed_img_path, img_size=(224, 224), cam_ratio=0.5):
    ...
    pred_score.backward()
    grad = torch.nn.functional.relu(gradients[0])
    features = features[0] * grad
    features = torch.mean(features, 0)
    cam_map = features.detach().cpu().numpy()
